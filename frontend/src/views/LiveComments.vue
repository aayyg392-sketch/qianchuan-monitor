...
const replyLogs = ref([
  { id: 1, category: 'inquiry', time: '14:32', user: '小红', comment: '这个有其他颜色吗？', reply: '亲～这款有白色、黑色、粉色三个颜色哦，都很好看呢！' },
  { id: 2, category: 'positive', time: '14:28', user: '购物达人', comment: '已收到，质量很好！', reply: '感谢您的反馈，很高兴您喜欢～欢迎下次再来！' },
  { id: 3, category: 'negative', time: '14:25', user: '用户123', comment: '发货太慢了', reply: '非常抱歉让您久等了，我们已加急处理，请您耐心等待～' },
])

const commentStats = ref({ total_today: 1256, ai_replies: 890, success_rate: 97.2, avg_response: '23s' })
const accountStats = ref([
  { key: 1, account: '好物推荐官', today: 320, ai: 245, success: '98%', risk: '正常' },
  { key: 2, account: '美妆小助手', today: 280, ai: 210, success: '96%', risk: '正常' },
  { key: 3, account: '食品旗舰店', today: 190, ai: 155, success: '97%', risk: '警告' },
])
...
const loadTasks = async () => {
  ...
  try {
    ...
    tasks.value = [
      { id: 1, name: '爆款视频评论引导', status: 'running', progress: 65, account_count: 5, video_count: 12, script_group: '正面引导', success_count: 780, fail_count: 12, filtered_count: 45, created_at: '2024-03-27 10:30' },
      { id: 2, name: '新品上架互动', status: 'pending', progress: 0, account_count: 3, video_count: 8, script_group: '产品卖点', success_count: 0, fail_count: 0, filtered_count: 0, created_at: '2024-03-27 11:00' },
      { id: 3, name: '口碑维护-售后', status: 'completed', progress: 100, account_count: 4, video_count: 6, script_group: '问题解答', success_count: 320, fail_count: 5, filtered_count: 18, created_at: '2024-03-26 09:15' },
    ]
...
const loadAccounts = async () => {
  try {
    ...
    accounts.value = [
      { id: 1, name: '好物推荐官', uid: 'douyin_123456' },
      { id: 2, name: '美妆小助手', uid: 'douyin_234567' },
      { id: 3, name: '食品旗舰店', uid: 'douyin_345678' },
      { id: 4, name: '日用百货铺', uid: 'douyin_456789' },
      { id: 5, name: '运动达人号', uid: 'douyin_567890' },
    ]
  }
}

const startTask = async (id) => { try { await request.post(`/live/comment-tasks/${id}/start`); message.success('任务已启动'); loadTasks() } catch { message.success('任务已启动(演示)') } }
const pauseTask = async (id) => { try { await request.post(`/live/comment-tasks/${id}/pause`); message.success('任务已暂停'); loadTasks() } catch { message.success('任务已暂停(演示)') } }
const deleteTask = async (id) => { try { await request.delete(`/live/comment-tasks/${id}`); message.success('已删除'); loadTasks() } catch { message.success('已删除(演示)'); tasks.value = tasks.value.filter(t => t.id !== id) } }
...
const submitTask = async () => {
  ...
  try {
    ...
    loadTasks()
  } catch { message.success('任务已创建(演示)'); showCreateTask.value = false }
...
watch(activeTab, async (v) => {
  if (v === '评论统计') {
    ...
    if (statsChartRef.value) {
      ...
      const days = ['03-21', '03-22', '03-23', '03-24', '03-25', '03-26', '03-27']
      statsChart.setOption({
        tooltip: { trigger: 'axis' },
        legend: { data: ['批量评论', 'AI回复', '成功率'] },
        grid: { left: 40, right: 40, top: 32, bottom: 24 },
        xAxis: { type: 'category', data: days },
        yAxis: [{ type: 'value', name: '数量' }, { type: 'value', name: '%', max: 100 }],
        series: [
          { name: '批量评论', type: 'bar', data: [320, 450, 380, 510, 420, 480, 560], itemStyle: { color: '#1677FF', borderRadius: [4, 4, 0, 0] } },
          { name: 'AI回复', type: 'bar', data: [210, 320, 280, 390, 310, 350, 420], itemStyle: { color: '#00B96B', borderRadius: [4, 4, 0, 0] } },
          { name: '成功率', type: 'line', yAxisIndex: 1, smooth: true, lineStyle: { color: '#FF8A00' }, itemStyle: { color: '#FF8A00' }, data: [96, 97, 95, 98, 97, 96, 97] },
        ]
      })
    }
  }
