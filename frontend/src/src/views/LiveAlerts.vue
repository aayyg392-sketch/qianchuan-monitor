...
const alertSummary = ref({ critical: 0, warning: 0, info: 0, resolved: 0 })
...
const loadAlerts = async () => {
  try {
    const res = await request.get('/live/alerts', { params: { page: 1, pageSize: 20 } })
    if (res && res.data) {
      const data = res.data
      if (data.summary) alertSummary.value = data.summary
      if (data.active_alerts) activeAlerts.value = data.active_alerts
      if (data.history) alertHistory.value = (data.history || []).map((h, i) => ({ ...h, key: h.key || i + 1 }))
      if (data.trend) {
...
const loadAlertConfigs = async () => {
  try {
    const res = await request.get('/live/alert-configs')
    if (res && res.data) {
      alertConfigs.value = res.data.configs || res.data || []
    }
  } catch (e) {
    alertConfigs.value = []
  }
...
const saveConfig = async () => {
  try {
    await request.post('/live/alert-configs', { configs: alertConfigs.value, notify: notifyConfig })
...
onMounted(async () => {
  await nextTick()
  await Promise.all([loadAlerts(), loadAlertConfigs()])
})
