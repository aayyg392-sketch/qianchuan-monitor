    ...
    <div class="material-grid" v-if="!loading && materials.length > 0">
      <div class="material-card" v-for="(item, idx) in materials" :key="item.material_id" @click="openDetail(item)">
        <div class="card-cover" :style="getCoverStyle(idx)">
          ...
          <span class="mat-status-tag" :class="'mst--' + getMaterialStatus(item)">{{ matStatusLabel(getMaterialStatus(item)) }}</span>
        ...
        <div class="card-metrics">
          ...
          <div class="metric-cell">
            <span class="m-label">成交</span>
            <span class="m-value" :class="parseInt(item.pay_order_count) > 0 ? 'green' : ''">{{ parseInt(item.pay_order_count || 0) }}单</span>
          </div>
          <div class="metric-cell">
            <span class="m-label">GMV</span>
            <span class="m-value green">&yen;{{ formatNum(item.pay_order_amount) }}</span>
          </div>
          <div class="metric-cell">
            <span class="m-label">GPM</span>
            <span class="m-value purple">{{ calcGpm(item) }}</span>
          </div>
        </div>
      </div>
    </div>
...
function getMaterialStatus(item) {
  ...
  const orders = parseInt(item.pay_order_count || 0)
  if (cost >= 500 && roi >= 2.0 && orders >= 10) return 'hot'
  if (roi >= 1.0 && orders > 0) return 'healthy'
  if (cost >= 50 && ((roi > 0 && roi < 0.5) || (cost >= 200 && orders === 0))) return 'bad'
  return 'observe'
}
function matStatusLabel(s) { return { hot: '爆款', healthy: '健康', observe: '观察', bad: '低效' }[s] || s }
function calcGpm(item) {
  const gmv = parseFloat(item.pay_order_amount || 0)
  const show = parseInt(item.product_show_count || item.show_cnt || 0)
  if (show <= 0) return '--'
  return (gmv / show * 1000).toFixed(0)
}
...
.mat-status-tag {
  ...
  border-radius: 4px; z-index: 2; letter-spacing: 1px;
}
.mst--hot { background: rgba(220, 38, 38, 0.9); color: #fff; }
.mst--healthy { background: rgba(22, 163, 74, 0.85); color: #fff; }
.mst--observe { background: rgba(0,0,0,0.35); color: #fff; }
.mst--bad { background: rgba(217, 119, 6, 0.9); color: #fff; }

.m-value.green { color: #00B96B; }
.m-value.purple { color: #722ED1; }
