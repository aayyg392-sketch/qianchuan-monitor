...
router.get('/list', async (req, res) => {
  try {
    ...
    // 统计
    const shopWhere = shop_id ? 'WHERE shop_id = ?' : '';
    ...
    const [[stats]] = await db.query(`
      SELECT
        ...
        SUM(CASE WHEN DATE(post_time) = CURDATE() THEN 1 ELSE 0 END) as today_new,
        SUM(CASE WHEN DATE(reply_time) = CURDATE() AND replied = 1 THEN 1 ELSE 0 END) as today_replied
      FROM ks_ad_comments ${shopWhere}
    `, shopParams);
    ...
    for (const acc of accounts) {
      try {
        ...
        const r = await ksAdApi('/v1/comment/list', acc.access_token, {
          ...
          post_start_time: threeDaysAgo,
          post_end_time: now,
          page: 1,
          page_size: 100,
        });

        logger.info(`[KS-AD-Comments] ${acc.advertiser_id} API返回: code=${r.code} msg=${r.message || ''} total=${r.data?.total || 0} details=${r.data?.details?.length || 0}`);
        ...
        if (r.code === 0 && r.data?.details?.length) {
          for (const c of r.data.details) {
            ...
            `, [
              ...
              c.nickname || c.nick_name || '', c.comment_content || '',
              postTime, c.fav_num || 0, c.comment_level || 1, c.shield_status || 1
            ]);
            totalSynced++;
          }
          logger.info(`[KS-AD-Comments] ${acc.advertiser_id} 入库 ${r.data.details.length} 条评论`);
        } else if (r.code !== 0) {
...
router.runAutoReply = runAutoReply;
router.getAdAccountsByShop = getAdAccountsByShop;
