  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT SUM(cost) as total_cost, SUM(cpm) as total_gmv, SUM(convert_cnt) as total_orders,
       ...
       FROM qc_daily_stats WHERE stat_date = ? AND entity_type = 'account' AND entity_id = '1713421159436366'`,
      [today]
    );
    return rows[0] || null;
  } catch (e) {
  ...
  try {
    ...
    for (const room of rooms) {
      try {
        ...
        // 5.5 获取Webcast在线人数补充全域推广缺失数据
        if (room.web_rid) {
          const webcast = await getWebcastOnlineCount(room.web_rid);
          if (webcast) {
            if (!data) data = { _source: "webcast", _is_living: webcast.status === 2 };
            data._webcast_online = webcast.online || 0;
            data._webcast_likes = webcast.likeCount || 0;
          }
        }
...
// 获取Webcast在线人数
async function getWebcastOnlineCount(webRid) {
  try {
    const r0 = await axios.get('https://live.douyin.com/', {
    ...
    const r = await axios.get('https://live.douyin.com/webcast/room/web/enter/', {
      params: { aid: 6383, app_name: 'douyin_web', live_id: 1, device_platform: 'web',
        language: 'zh-CN', browser_language: 'zh-CN', browser_platform: 'Win32',
        browser_name: 'Chrome', browser_version: '120.0.0.0', web_rid: webRid },
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        'Referer': 'https://live.douyin.com/' + webRid, 'Cookie': cookies },
      timeout: 8000,
    });
    const d = r.data?.data?.data?.[0];
    if (d) {
      return {
        online: parseInt(d.user_count_str || d.user_count || 0),
        likeCount: d.like_count || 0,
        status: d.status,
      };
    }
  } catch (e) { logger.warn('[LiveSync] Webcast在线人数获取失败', { error: e.message }); }
  return null;
}
function buildRealtimeRecord(room, apiData, adStats, prev) {
  ...
  if (apiData && apiData._source === 'uni_promotion') {
    ...
    return {
      online_count: apiData._webcast_online || (apiData._is_living ? Math.max(prev?.online_count || 1, 1) : 0),
      enter_count: estimatedPaidUV,
      leave_count: 0,
      // 当只有paid数据时，按经验比例估算各流量来源
      total_viewers: estimatedPaidUV > 0 ? (estimatedPaidUV + Math.round(estimatedPaidUV * 0.5) + Math.round(estimatedPaidUV * 0.25) + Math.round(estimatedPaidUV * 0.15) + Math.round(estimatedPaidUV * 0.1)) : 0,
      peak_count: Math.max(prev?.peak_count || 0, prev?.online_count || 0),
      avg_stay_seconds: estimatedPaidUV > 0 && (apiData._webcast_online || 0) > 0 ? Math.round((apiData._webcast_online * 99) / Math.max(1, estimatedPaidUV / 3600)) : (prev?.avg_stay_seconds || 99),
      interact_rate: totalConverts > 0 && estimatedPaidUV > 0 ? (totalConverts / estimatedPaidUV * 100) : 0,
      comment_count: 0,
      like_count: apiData._webcast_likes || 0,
      share_count: 0,
      ...
      uv_value: uvValue,
      gpm: gpm,
      source_organic: estimatedPaidUV > 0 ? Math.round(estimatedPaidUV * 0.5) : 0,
      source_paid: estimatedPaidUV,
      source_video: estimatedPaidUV > 0 ? Math.round(estimatedPaidUV * 0.25) : 0,
      source_search: estimatedPaidUV > 0 ? Math.round(estimatedPaidUV * 0.1) : 0,
      source_follow: estimatedPaidUV > 0 ? Math.round(estimatedPaidUV * 0.15) : 0,
