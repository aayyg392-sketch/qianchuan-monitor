...
async function sendBriefingToDingTalk(content, date) {
  ...
  try {
    ...
    logger.info('[Briefing] 钉钉群机器人简报发送成功');
  } catch (e) {
    logger.error('[Briefing] 钉钉群机器人发送失败', { error: e.message });
  }

  // 2. 发机器人单聊（独立聊天窗口）
  const BRIEFING_USER_ID = '062134001437905837';
  try {
    const { sendRobotMessage } = require('./dingtalk');
    await sendRobotMessage(BRIEFING_USER_ID, title, text);
    logger.info(`[Briefing] 机器人单聊日报已发送: ${BRIEFING_USER_ID}`);
  } catch (e) {
    logger.error('[Briefing] 机器人单聊发送失败', { error: e.message });
  }
}
