 ...
 */
const { SCHEDULER_CONFIG } = require('./config');
const adqConnector = require('./connectors/adq');
...
class AIScheduler {
  ...
  async _mainLoop() {
    ...
    try {
      ...
      // 查找所有AI接管的ADQ账户并逐个执行
      const [takeoverRules] = await this.db.query(
        "SELECT * FROM ai_rules WHERE rule_type = 'ai_takeover' AND is_active = 1 AND platform = 'adq'"
      );

      for (const rule of takeoverRules) {
        try {
          const cfg = typeof rule.rule_config === 'string' ? JSON.parse(rule.rule_config) : rule.rule_config;
          if (cfg?.accountDbId) {
            await adqConnector.runForAccount(cfg.accountDbId);
          }
        } catch (e) {
          this.logger.error(`AI引擎处理账户失败`, { ruleId: rule.id, error: e.message });
        }
      }
    ...
    try {
      ...
      try {
        const [todayData] = await this.db.query(
          `SELECT * FROM ai_metrics WHERE platform = 'adq' AND metric_type = 'daily_snapshot' AND DATE(recorded_at) = CURDATE() ORDER BY recorded_at DESC LIMIT 1`
        );
        if (todayData.length && todayData[0].metric_data) {
          ...
          // 具体异常检测由adq connector在runForAccount中执行
        }
      } catch (e) { /* skip */ }
    } catch (e) {
